...
import { RESTAURANT_BUCKET_ID, APPWRITE_PROJECT_ID } from '../config/environment';
...
export const uploadImage = async (req: Request, res: Response): Promise<void> => {
  if (req.file) {
    ...
    storage
      .createFile(RESTAURANT_BUCKET_ID, ID.unique(), file)
      .then((imageResponse) => {
        const imageUrl = `https://cloud.appwrite.io/v1/storage/buckets/${RESTAURANT_BUCKET_ID}/files/${imageResponse.$id}/view?project=${APPWRITE_PROJECT_ID}`;
